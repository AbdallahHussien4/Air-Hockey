...
KEYCHECK MACRO scan
    local UP1,DUMMY16,DUMMY15,DUMMY2,DUMMY1,check,startS,Down1,black2,start2,UP2,black3,start3,DOWN2,start3,start4,black4,exit,dummy35,dummy36 
    push cx
    push dx 
    pushf
    
check:   
           CMP scan,72              ;Up arrow
           jz dummy1
           CMP scan,80              ;Down arrow
           JZ Dummy2
           CMP scan,11h             ;W
           JZ  DUMMY35              
           CMP scan,1Fh             ;S
...
;Initialize playing Page,Status bar and chatting space          
            ...
            mov ah,9
            mov dx,offset PlayerName   ;Player 2 name
            int 21H 
...
GetSpace:                  ;Waiting for space key to start the game
    mov ah,0
    int 16h 
    cmp al,32
    jnz GetSpace        
